        ...
        case CB_MSG_TYPE:
			...
			/* Elvégzi az adatfeldolgozást */
 			nType = sTI[site_inx].nType;
 				...
 				MOSCAD_sprintf(message,"Frame received, index: %d, type: %d, rx_buffer[0]: %d,rx_buffer[8]: %d, length: %d",site_inx,nType,nRxBuf[0],nRxBuf[8],buff_len);
   			 	MOSCAD_error(message ); 				
...
ReteszParancsokKezdoCim = 230;	/* DC4, 200 */																		/**/
																													/**/
ReteszesTMOKNum = 112;					      /* Ennyi reteszfeltételes TMOK van az adott front-endben*/					/**/
...
ReteszesRTUIndex[0][1] = 185;			/* Szany 0115/6  PV erõmû */															/**/

TMOK_ID[0][2] =1250;						/* TMOK azonosítója a táviratban = DP offset */								/**/															
ReteszesRTUIndex[0][2] = 186;			/* Szany 0115/7  PV erõmû */															/**/
...
/* 2. TMOK: Front end D -> 62-78; RTU: Fertõd, naperõmû.Fertõd 072/14,15,16,19,20 PV erõmû  -----------------------*/								/**/
...
TMOK_ID[2][1] =1251;						/* TMOK azonosítója a táviratban = DP offset */								/**/															
ReteszesRTUIndex[2][1] = 148;			/* Fertõd 072/14,15,16,19,20 PV erõmû */															/**/
...
nReteszPar[2] = 0;                /* 1: tartozik hozzá DC parancs, 0: nem tartozik hozzá DC parancs */
																													/**/
/* 3. TMOK: Front end D -> 61-70; RTU: Fertõd, naperõmû, Fertõszéplak 424/5,Fertõd 072/14,15,16,19,20 PV erõmû  -----------------------*/								/**/
...
TMOK_ID[3][1] =1252;						/* TMOK azonosítója a táviratban = DP offset */								/**/															
ReteszesRTUIndex[3][1] = 128;			/* Fertõszéplak 424/5 */															/**/

TMOK_ID[3][1] =1252;						/* TMOK azonosítója a táviratban = DP offset */								/**/															
ReteszesRTUIndex[3][1] = 148;			/* Fertõd 072/14,15,16,19,20 PV erõmû */															/**/
...
nReteszPar[3] = 0;                /* 1: tartozik hozzá DC parancs, 0: nem tartozik hozzá DC parancs */
																													/**/
/* 4. TMOK: 21074 RTU:  Kesztölc, naperõmû I. II. Kesztölc 0773-----------------------*/								/**/
...
TMOK_ID[4][2] =1253;						/* TMOK azonosítója a kmenõ táviratban = DP offset */								/**/															

ReteszesRTUIndex[4][0] = 72;			/* Kesztölc 032-18 */															/**/
ReteszesRTUIndex[4][1] = 73;			/* Kesztölc 032-21 */															/**/
ReteszesRTUIndex[4][2] =162;			/* Kesztölc 3733 */															/**/

ReteszesTMOK_RTUNum[4] = 3;				/* Az adott indexû TMOK ennyi kábelköri állomnással kommunikál */			/**/
nReteszPar[4] = 0;                /* 1: tartozik hozzá DC parancs, 0: nem tartozik hozzá DC parancs */
                                                          
/* 5. TMOK: 13-30 RTU:  Tét, 055/36, 055/39 PV erõmû               -----------------------*/								/**/
...
ReteszesRTUIndex[5][0] = 78;			/* Tét 055-36 */															/**/
...
ReteszesRTUIndex[6][0] = 78;			/* Tét 055-36 */															/**/

TMOK_ID[6][1] =1255;						/* TMOK azonosítója a kmenõ táviratban = DP offset */								/**/															
ReteszesRTUIndex[6][1] = 122;			/* Tét 055-39 */															/**/

ReteszesTMOK_RTUNum[6] = 2;				/* Az adott indexû TMOK ennyi kábelköri állomnással kommunikál */			/**/
nReteszPar[6] = 0;                /* 1: tartozik hozzá DC parancs, 0: nem tartozik hozzá DC parancs */

/* 7. TMOK: 11-35 RTU:  Tét, 055/36, 055/39 PV erõmû               -----------------------*/								/**/
...
ReteszesRTUIndex[7][0] = 78;			/* Tét 055-36 */															/**/

TMOK_ID[7][1] =1256;						/*  TMOK azonosítója a kmenõ táviratban = DP offset */								/**/															
ReteszesRTUIndex[7][1] = 122;			/* Tét 055-39 */															/**/
...
/* 16. TMOK: 32-24 RTU: Alcsút 071-11 PV erõmû, Alcsút 071-11/2 PV erõmû  - tesztelve            -----------------------*/								/**/
...
TMOK_ID[16][1] =1264;						         /*  TMOK azonosítója a kmenõ táviratban = DP offset */								/**/															
ReteszesRTUIndex[16][1] = 146;			   /* Alcsút 071-11 PV erõmû */															/**/
...
/* 17. TMOK: 87-35 RTU: Alcsút 071-11 PV erõmû, Bicske 041/8, Alcsút 071-11/2 PV erõmû   -  tesztelve           -----------------------*/								/**/
...
TMOK_ID[17][2] =1265;						         /*  TMOK azonosítója a kmenõ táviratban = DP offset */								/**/															
ReteszesRTUIndex[17][2] = 146;			   /* Bicske 041/8 PV erõmû */															/**/

ReteszesTMOK_RTUNum[17] = 3;				   /* Az adott indexû TMOK ennyi kábelköri állomnással kommunikál */			/**/
...
/* 24. TMOK: 71-39 RTU: Magyargencs 184 hrsz. PV erõmû, Magyargencs 185 hrsz.  -  Tesztelve           -----------------------*/								/**/
...
TMOK_ID[24][1] =1272;						         /*  TMOK azonosítója a kmenõ táviratban = DP offset */								/**/															
ReteszesRTUIndex[24][1] = 188;			   /* Magyargencs 185 hrsz. PV erõmû   */															/**/
...
/* 25. TMOK: 60-71 RTU: Magyargencs 184 hrsz. PV erõmû, Szany 0155/6, Szany 0155/7 PV erõmû, Magyargencs 185   -   Tesztelve          -----------------------*/								/**/
...
TMOK_ID[25][1] =1273;						         /*  TMOK azonosítója a kmenõ táviratban = DP offset */								/**/															
ReteszesRTUIndex[25][1] = 185;			   /* Szany 077/6 PV erõmû   */															/**/

TMOK_ID[25][2] =1273;						         /*  TMOK azonosítója a kmenõ táviratban = DP offset */								/**/															
ReteszesRTUIndex[25][2] = 186;			   /* Szany 077/7 PV erõmû   */															/**/

TMOK_ID[25][3] =1273;						         /*  TMOK azonosítója a kmenõ táviratban = DP offset */								/**/															
ReteszesRTUIndex[25][3] = 188;			   /* Magyargencs 185PV erõmû   */															/**/
...
/* 26. TMOK: 72-44 RTU: Magyargencs 184 hrsz. PV erõmû, Magyargencs 185 hrsz.  -   Tesztelve          -----------------------*/								/**/
...
TMOK_ID[26][1] =1274;						         /*  TMOK azonosítója a kmenõ táviratban = DP offset */								/**/															
ReteszesRTUIndex[26][1] = 188;			   /* Magyargencs 185 hrsz. PV erõmû   */															/**/
...
/* 27. TMOK: 72-29 RTU: Magyargencs 184 hrsz. PV erõmû, Magyargencs 185 hrsz.  -  Tesztelve           -----------------------*/								/**/
...
TMOK_ID[27][1] = 592;						         /*  TMOK azonosítója a kmenõ táviratban = DP offset */								/**/															
ReteszesRTUIndex[27][1] = 188;			   /* Magyargencs 185 hrsz. PV erõmû   */															/**/

ReteszesTMOK_RTUNum[27] = 2;				   /* Az adott indexû TMOK ennyi kábelköri állomnással kommunikál */			/**/
...
/* 35. TMOK: 61-81 RTU: Fertõszéplak 242/2, Fertõszéplak 424/5  PV erõmû, Pereszteg 0160/2  - Tesztelve            -----------------------*/								/**/
...
TMOK_ID[35][2] =1282;						         /*  TMOK azonosítója a kmenõ táviratban = DP offset */								/**/															
ReteszesRTUIndex[35][2] = 187;			   /* Pereszteg 0160/2 PV erõmû   */															/**/

ReteszesTMOK_RTUNum[35] = 3;				   /* Az adott indexû TMOK ennyi kábelköri állomnással kommunikál */			/**/
...
/* 40. TMOK: 37-63 RTU: Várpalota 4134/3  PV erõmû  - Tesztelve            -----------------------*/								/**/
TMOKAllasjelzesOffsetek[40] = 610; 	 /* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[40][0] = 610;						         /*  TMOK azonosítója a kmenõ táviratban = DP offset */								/**/															
ReteszesRTUIndex[40][0] = 111;			   /* Várpalota 4134/3 PV erõmû   */															/**/
...
nReteszOffset[40] = 11;             /* DC parancs és DP állapot offsete, ha tartozik hozzá DC parancs*/

/* 41. TMOK: 37-65 RTU: Várpalota 4134/3  PV erõmû, Várpalota 0192/4  - Tesztelve            -----------------------*/								/**/
TMOKAllasjelzesOffsetek[41] = 611; 	 /* Az állásjelzés offsete a DP adatbázisban */								/**/

TMOK_ID[41][0] = 611;						         /*  TMOK azonosítója a kmenõ táviratban = DP offset */								/**/															
ReteszesRTUIndex[41][0] = 111;			   /* Várpalota 4134/3 PV erõmû   */															/**/

TMOK_ID[41][1] = 611;						         /*  TMOK azonosítója a kmenõ táviratban = DP offset */								/**/															
ReteszesRTUIndex[41][1] = 143;			   /* Várpalota 0192/4 PV erõmû   */															/**/
...
nReteszOffset[41] = 12;             /* DC parancs és DP állapot offsete, ha tartozik hozzá DC parancs*/

/* 42. TMOK: 37-66 RTU: Várpalota 4134/3  PV erõmû, Várpalota 0192/4 - Tesztelve            -----------------------*/								/**/
TMOKAllasjelzesOffsetek[42] = 612; 	 /* Az állásjelzés offsete a DP adatbázisban */								/**/

TMOK_ID[42][0] = 612;						         /*  TMOK azonosítója a kmenõ táviratban = DP offset */								/**/															
ReteszesRTUIndex[42][0] = 111;			   /* Várpalota 4134/3 PV erõmû   */															/**/

TMOK_ID[42][1] = 612;						         /*  TMOK azonosítója a kmenõ táviratban = DP offset */								/**/															
ReteszesRTUIndex[42][1] = 143;			   /* Várpalota 0192/4 PV erõmû   */															/**/

ReteszesTMOK_RTUNum[42] = 2;				   /* Az adott indexû TMOK ennyi kábelköri állomnással kommunikál */			/**/
...
/* 44. TMOK: 52-51 RTU: Vigánpetend 012/1_1,2  Tapolca 0350/34-37, Tapolca 0350/38-40, Tapolca 0350/42-48 PV erõmû  - Tesztelve            -----------------------*/								/**/
...
TMOK_ID[44][2] = 613;						         /*  TMOK azonosítója a kmenõ táviratban = DP offset */								/**/															
ReteszesRTUIndex[44][2] = 152;			   /* Tapolca 0350/38,39,40 PV erõmû   */															/**/

TMOK_ID[44][3] = 613;						         /*  TMOK azonosítója a kmenõ táviratban = DP offset */								/**/															
ReteszesRTUIndex[44][3] = 155;			   /* Tapolca 0366/42-48 PV erõmû    */															/**/

TMOK_ID[44][4] = 613;						         /*  TMOK azonosítója a kmenõ táviratban = DP offset */								/**/															
ReteszesRTUIndex[44][4] = 161;			   /* Tapolca 0366/34-37 PV erõmû    */															/**/


ReteszesTMOK_RTUNum[44] = 5;				   /* Az adott indexû TMOK ennyi kábelköri állomnással kommunikál */			/**/
...
/* 49. TMOK: 13-20 RTU: Gyõr 0747/9, Gönyû 1182  PV erõmû  - Tesztelve            -----------------------*/								/**/
...
TMOK_ID[49][1] = 1293;						         /*  TMOK azonosítója a kmenõ táviratban = DP offset */								/**/															
ReteszesRTUIndex[49][1] = 124;			   /* Gönyû 1182 PV erõmû   */															/**/

ReteszesTMOK_RTUNum[49] = 2;				   /* Az adott indexû TMOK ennyi kábelköri állomnással kommunikál */			/**/
nReteszPar[49] = 0;                   /* 1: tartozik hozzá DC parancs, 0: nem tartozik hozzá DC parancs */

/* 50. TMOK: 13-10 RTU: Gönyû 1182  PV erõmû  - Tesztelve            -----------------------*/								/**/
TMOKAllasjelzesOffsetek[50] = 1294; 	 /* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[50][0] = 1294;						         /*  TMOK azonosítója a kmenõ táviratban = DP offset */								/**/															
ReteszesRTUIndex[50][0] = 124;			   /* Gönyû 1182 PV erõmû   */															/**/
...
/* 51. TMOK: 10-01 RTU: Gönyû 1182  PV erõmû  - Tesztelve            -----------------------*/								/**/
TMOKAllasjelzesOffsetek[51] = 1295; 	 /* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[51][0] = 1295;						         /*  TMOK azonosítója a kmenõ táviratban = DP offset */								/**/															
ReteszesRTUIndex[51][0] = 124;			   /* Gönyû 1182 PV erõmû   */															/**/
...
TMOKAllasjelzesOffsetek[53] = 1297; 	 /* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[53][0] = 1297;						         /*  TMOK azonosítója a kmenõ táviratban = DP offset */								/**/															
ReteszesRTUIndex[53][0] = 125;			   /* Fertõrákos 030/5 PV erõmû   */															/**/
...
/* 55. TMOK: 90-65 RTU: Zalavég 044/3  PV erõmû  - Tesztelve            -----------------------*/								/**/
TMOKAllasjelzesOffsetek[55] = 1298; 	 /* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[55][0] = 1298;						         /*  TMOK azonosítója a kmenõ táviratban = DP offset */								/**/															
...
/* 64. TMOK: 10-93 RTU: Gyõr 452/6 Árki  PV erõmû  - Tesztelve            -----------------------*/								/**/
TMOKAllasjelzesOffsetek[64] = 1305; 	 /* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[64][0] = 1305;						         /*  TMOK azonosítója a kmenõ táviratban = DP offset */								/**/															
ReteszesRTUIndex[64][0] = 135;			   /* Gyõr 452/6 Árki  PV erõmû   */															/**/
...
ReteszesRTUIndex[65][0] = 135;			   /* Gyõr 452/6 Árki  PV erõmû   */															/**/
...
TMOKAllasjelzesOffsetek[66] = 1307; 	 /* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[66][0] = 1307;						         /*  TMOK azonosítója a kmenõ táviratban = DP offset */								/**/															
ReteszesRTUIndex[66][0] = 135;			   /* Gyõr 452/6 Árki  PV erõmû   */															/**/
...
/* 73. TMOK: 72-91 RTU: Kolontár 321, Tapolca 0258, 0259  PV erõmû  - Tesztelve            -----------------------*/								/**/
...
TMOK_ID[73][1] = 1313;						         /*  TMOK azonosítója a kmenõ táviratban = DP offset */								/**/															
ReteszesRTUIndex[73][1] = 202;			   /* Tapolca 0258, 0259 PV erõmû   */															/**/

ReteszesTMOK_RTUNum[73] = 2;				   /* Az adott indexû TMOK ennyi kábelköri állomnással kommunikál */			/**/
nReteszPar[73] = 0;                   /* 1: tartozik hozzá DC parancs, 0: nem tartozik hozzá DC parancs */

/* 74. TMOK: 61-59 RTU: Fertõd 072/14,15,16,19,20 PV erõm  PV erõmû  - Tesztelve            -----------------------*/								/**/
TMOKAllasjelzesOffsetek[74] = 1314; 	 /* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[74][0] = 1314;						         /*  TMOK azonosítója a kmenõ táviratban = DP offset */								/**/															
ReteszesRTUIndex[74][0] = 148;			   /* Fertõd 072/14,15,16,19,20 PV erõm  */															/**/
ReteszesTMOK_RTUNum[74] = 1;				   /* Az adott indexû TMOK ennyi kábelköri állomnással kommunikál */			/**/
nReteszPar[74] = 0;                   /* 1: tartozik hozzá DC parancs, 0: nem tartozik hozzá DC parancs */

/* 75. TMOK: 64-29 RTU: Fertõd 072/14,15,16,19,20 PV erõm  PV erõmû  PV erõmû  - Tesztelve            -----------------------*/								/**/
TMOKAllasjelzesOffsetek[75] = 946; 	 /* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[75][0] = 946;						         /*  TMOK azonosítója a kmenõ táviratban = DP offset */								/**/															
ReteszesRTUIndex[75][0] = 148;			   /* Fertõd 072/14,15,16,19,20 PV erõm  PV erõmû   */															/**/
ReteszesTMOK_RTUNum[75] = 1;				   /* Az adott indexû TMOK ennyi kábelköri állomnással kommunikál */			/**/
nReteszPar[75] = 1;                   /* 1: tartozik hozzá DC parancs, 0: nem tartozik hozzá DC parancs */
nReteszOffset[75] = 19;             /* DC parancs és DP állapot offsete, ha tartozik hozzá DC parancs*/
...
/* 77. TMOK: 51-40 RTU: Tapolca 0350/38,39,40, Tapolca 0366/42-48 PV erõmû  - Tesztelve            -----------------------*/								/**/
...
TMOK_ID[77][2] = 1315;						         /*  TMOK azonosítója a kmenõ táviratban = DP offset */								/**/															
ReteszesRTUIndex[77][2] = 161;			   /* Tapolca 0366/34-37 PV erõmû    */															/**/
...
TMOK_ID[79][1] = 1317;						         /*  TMOK azonosítója a kmenõ táviratban = DP offset */								/**/															
ReteszesRTUIndex[79][1] = 155;			   /* Tapolca 0366/42-48 PV erõmû    */															/**/

TMOK_ID[79][2] = 1317;						         /*  TMOK azonosítója a kmenõ táviratban = DP offset */								/**/															
ReteszesRTUIndex[79][2] = 161;			   /* Tapolca 0366/34-37 PV erõmû    */															/**/

ReteszesTMOK_RTUNum[79] = 3;				   /* Az adott indexû TMOK ennyi kábelköri állomnással kommunikál */			/**/
...
TMOK_ID[81][0] =  6;						/* TMOK azonosítója a táviratban = DP offset */								/**/															
ReteszesRTUIndex[81][0] = 65;			/* B redundancia */															/**/
ReteszesTMOK_RTUNum[81] = 1;				/* Az adott indexû TMOK ennyi kábelköri állomnással kommunikál */			/**/
nReteszPar[81] = 1;                /* 1: tartozik hozzá DC parancs, 0: nem tartozik hozzá DC parancs */
nReteszOffset[81] = 22;             /* DC parancs offsete, ha tartozik hozzá DC parancs*/

/* 82. TMOK:64-57 H front end ->   RTU: H: Sobor 077/6, Sobor 077/7 PV, Szany 0155/6 , Szany 0155/7 									/**/
TMOKAllasjelzesOffsetek[82] = 1023; 		/* Az állásjelzés offsete a DP adatbázisban */								/**/

TMOK_ID[82][0] =  1023;						/* TMOK azonosítója a táviratban = DP offset */								/**/															
ReteszesRTUIndex[82][0] = 158;			/* Sobor 077/6 */															/**/

TMOK_ID[82][1] =  1023;						/* TMOK azonosítója a táviratban = DP offset */								/**/															
ReteszesRTUIndex[82][1] = 159;			/* Sobor 077/7 */															/**/

TMOK_ID[82][2] = 1023;						         /*  TMOK azonosítója a kmenõ táviratban = DP offset */								/**/															
ReteszesRTUIndex[82][2] = 185;			   /* Szany 0155/6 PV erõmû   */															/**/

TMOK_ID[82][3] = 1023;						         /*  TMOK azonosítója a kmenõ táviratban = DP offset */								/**/															
ReteszesRTUIndex[82][3] = 186;			   /* Szany 0155/7 PV erõmû   */															/**/

ReteszesTMOK_RTUNum[82] = 4;				/* Az adott indexû TMOK ennyi kábelköri állomnással kommunikál */			/**/
nReteszPar[82] = 1;                /* 1: tartozik hozzá DC parancs, 0: nem tartozik hozzá DC parancs */
nReteszOffset[82] = 23;             /* DC parancs offsete, ha tartozik hozzá DC parancs*/

/* 83. TMOK 60-75 B Front end -> RTU: H: Sobor 077/6, Sobor 077/7 PV Szany 0155/6, Szany 0155/7  - Tesztelve            -----------------------*/								/**/
TMOKAllasjelzesOffsetek[83] = 1318; 	 /* Az állásjelzés offsete a DP adatbázisban */								/**/

TMOK_ID[83][0] = 1318;						         /*  TMOK azonosítója a kmenõ táviratban = DP offset */								/**/															
ReteszesRTUIndex[83][0] = 158;			   /* Sobor 077/6 PV erõmû   */															/**/

TMOK_ID[83][1] = 1318;						         /*  TMOK azonosítója a kmenõ táviratban = DP offset */								/**/															
ReteszesRTUIndex[83][1] = 159;			   /* Sobor 077/7 PV erõmû    */															/**/

TMOK_ID[83][2] = 1318;						         /*  TMOK azonosítója a kmenõ táviratban = DP offset */								/**/															
ReteszesRTUIndex[83][2] = 185;			   /* Szany 0155/6 PV erõmû   */															/**/

TMOK_ID[83][3] = 1318;						         /*  TMOK azonosítója a kmenõ táviratban = DP offset */								/**/															
ReteszesRTUIndex[83][3] = 186;			   /* Szany 0155/7 PV erõmû    */															/**/

ReteszesTMOK_RTUNum[83] = 4;				   /* Az adott indexû TMOK ennyi kábelköri állomnással kommunikál */			/**/
nReteszPar[83] = 0;                   /* 1: tartozik hozzá DC parancs, 0: nem tartozik hozzá DC parancs */

/* 84. TMOK 64-20 B Front end -> RTU: H: Sobor 077/6, Sobor 077/7 PV, Szany 0155/6, Szany 0155/7  - Tesztelve            -----------------------*/								/**/
TMOKAllasjelzesOffsetek[84] = 1319; 	 /* Az állásjelzés offsete a DP adatbázisban */								/**/

TMOK_ID[84][0] = 1319;						         /*  TMOK azonosítója a kmenõ táviratban = DP offset */								/**/															
ReteszesRTUIndex[84][0] = 158;			   /* Sobor 077/6  PV erõmû   */															/**/

TMOK_ID[84][1] = 1319;						         /*  TMOK azonosítója a kmenõ táviratban = DP offset */								/**/															
ReteszesRTUIndex[84][1] = 159;			   /* Sobor 077/7 PV erõmû    */															/**/

TMOK_ID[84][2] = 1319;						         /*  TMOK azonosítója a kmenõ táviratban = DP offset */								/**/															
ReteszesRTUIndex[84][2] = 185;			   /* Szany 0155/6  PV erõmû   */															/**/

TMOK_ID[84][3] = 1319;						         /*  TMOK azonosítója a kmenõ táviratban = DP offset */								/**/															
ReteszesRTUIndex[84][3] = 186;			   /* Szany 0155/7 PV erõmû    */															/**/

ReteszesTMOK_RTUNum[84] = 4;				   /* Az adott indexû TMOK ennyi kábelköri állomnással kommunikál */			/**/
...
/* 88. TMOK: 54-27 RTU: Hidegkút 07/22,23 PV erõmû - Tesztelve            -----------------------*/								/**/
TMOKAllasjelzesOffsetek[88] = 1074; 	 /* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[88][0] = 1074;						         /*  TMOK azonosítója a kmenõ táviratban = DP offset */								/**/															
ReteszesRTUIndex[88][0] = 165;			   /* Hidegkút 07/22,23 PV erõmû   */															/**/
...
/* 93. TMOK: Nemesgulács 3. leág. RTU: Tapolca 4 leág. PV erõmû, 59297 TMOK - Tesztelve            -----------------------*/								/**/
...
TMOK_ID[93][1] = 1107;						         /*  TMOK azonosítója a kmenõ táviratban = DP offset */								/**/															
ReteszesRTUIndex[93][1] = 180;			   /* TMOK 59297 */															/**/

ReteszesTMOK_RTUNum[93] = 2;				   /* Az adott indexû TMOK ennyi kábelköri állomnással kommunikál */			/**/
...
/* 94. TMOK: 52-62 RTU: Tapolca 4 leág. PV erõmû, 59297 TMOK - Tesztelve            -----------------------*/								/**/
...
TMOK_ID[94][1] = 1327;						         /*  TMOK azonosítója a kmenõ táviratban = DP offset */								/**/															
ReteszesRTUIndex[94][1] = 180;			   /* TMOK 59297 */															/**/

ReteszesTMOK_RTUNum[94] = 2;				   /* Az adott indexû TMOK ennyi kábelköri állomnással kommunikál */			/**/
nReteszPar[94] = 0;                   /* 1: tartozik hozzá DC parancs, 0: nem tartozik hozzá DC parancs */

/* 95. TMOK: 40-60 RTU: Radóckölked 258 PV erõmû, Nagykölked 04/3 PV erõmû - Tesztelve            -----------------------*/								/**/
TMOKAllasjelzesOffsetek[95] = 1328; 	 /* Az állásjelzés offsete a DP adatbázisban */								/**/

TMOK_ID[95][0] = 1328;						         /*  TMOK azonosítója a kmenõ táviratban = DP offset */								/**/															
ReteszesRTUIndex[95][0] = 176;			   /* Radóckölked 258 PV erõmû */															/**/

TMOK_ID[95][1] = 1328;						         /*  TMOK azonosítója a kmenõ táviratban = DP offset */								/**/															
ReteszesRTUIndex[95][1] = 192;			   /* Nagykölked 041/3 PV erõmû */															/**/

ReteszesTMOK_RTUNum[95] = 2;				   /* Az adott indexû TMOK ennyi kábelköri állomnással kommunikál */			/**/
nReteszPar[95] = 0;                   /* 1: tartozik hozzá DC parancs, 0: nem tartozik hozzá DC parancs */

/* 96. TMOK: 40-68 RTU: Radóckölked 258 PV erõmû,  Nagykölked 04/3, Nemesrempehollós 259/7 07/4 PV erõmû - - Tesztelve            -----------------------*/								/**/
...
TMOK_ID[96][1] = 1329;						         /*  TMOK azonosítója a kmenõ táviratban = DP offset */								/**/															
ReteszesRTUIndex[96][1] = 192;			   /* Nagykölked 041/3 PV erõmû */															/**/

TMOK_ID[96][2] = 1329;						         /*  TMOK azonosítója a kmenõ táviratban = DP offset */								/**/															
ReteszesRTUIndex[96][2] = 203;			   /* Nemesrempehollós 259/7 07/4 PV erõmû */															/**/

ReteszesTMOK_RTUNum[96] = 3;				   /* Az adott indexû TMOK ennyi kábelköri állomnással kommunikál */			/**/
...
/* 98. TMOK: 87-66 RTU: Bicske 071/2 PV erõmû - Tesztelve            -----------------------*/								/**/
TMOKAllasjelzesOffsetek[98] = 1331; 	 /* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[98][0] = 1331;						         /*  TMOK azonosítója a kmenõ táviratban = DP offset */								/**/															
ReteszesRTUIndex[98][0] = 179;			   /* Bicske 071/2 PV erõmû */															/**/
ReteszesTMOK_RTUNum[98] = 1;				   /* Az adott indexû TMOK ennyi kábelköri állomnással kommunikál */			/**/
nReteszPar[98] = 0;                   /* 1: tartozik hozzá DC parancs, 0: nem tartozik hozzá DC parancs */

/* 99. TMOK: 62-20 RTU: Osli 0100/22, Osli 0100/24 PV erõmû - Tesztelve            -----------------------*/								/**/
TMOKAllasjelzesOffsetek[99] = 1332; 	 /* Az állásjelzés offsete a DP adatbázisban */								/**/

TMOK_ID[99][0] = 1332;						         /*  TMOK azonosítója a kmenõ táviratban = DP offset */								/**/															
ReteszesRTUIndex[99][0] = 181;			   /* Osli 0100/22 PV erõmû */															/**/

TMOK_ID[99][1] = 1332;						         /*  TMOK azonosítója a kmenõ táviratban = DP offset */								/**/															
ReteszesRTUIndex[99][1] = 182;			   /* Osli 0100/24 PV erõmû */															/**/
...
/* 103. TMOK: 60-70 RTU: Szany 0155/6, Szany 0155/7 PV erõmû - Tesztelve            -----------------------*/								/**/
TMOKAllasjelzesOffsetek[103] = 1335; 	 /* Az állásjelzés offsete a DP adatbázisban */								/**/

TMOK_ID[103][0] = 1335;						         /*  TMOK azonosítója a kmenõ táviratban = DP offset */								/**/															
ReteszesRTUIndex[103][0] = 185;			   /* Szany 155/6 PV erõmû */															/**/

TMOK_ID[103][1] = 1335;						         /*  TMOK azonosítója a kmenõ táviratban = DP offset */								/**/															
ReteszesRTUIndex[103][1] = 186;			   /* Szany 155/7 PV erõmû */															/**/

ReteszesTMOK_RTUNum[103] = 2;				   /* Az adott indexû TMOK ennyi kábelköri állomnással kommunikál */			/**/
nReteszPar[103] = 0;                   /* 1: tartozik hozzá DC parancs, 0: nem tartozik hozzá DC parancs */

/* 104. TMOK: 64-57 RTU: Szany 0155/6, Szany 0155/7  PV erõmû  - Tesztelve            -----------------------*/								/**/
TMOKAllasjelzesOffsetek[104] = 206; 	 /* Az állásjelzés offsete a DP adatbázisban */								/**/

TMOK_ID[104][0] = 206;						         /*  TMOK azonosítója a kmenõ táviratban = DP offset */								/**/															
ReteszesRTUIndex[104][0] = 185;			   /* Szany 0155/6 PV erõmû   */															/**/

TMOK_ID[104][1] = 206;						         /*  TMOK azonosítója a kmenõ táviratban = DP offset */								/**/															
ReteszesRTUIndex[104][1] = 186;			   /* Szany 0155/7 PV erõmû   */															/**/


ReteszesTMOK_RTUNum[104] = 0;				   /* Az adott indexû TMOK ennyi kábelköri állomnással kommunikál */			/**/
nReteszPar[104] = 1;                   /* 1: tartozik hozzá DC parancs, 0: nem tartozik hozzá DC parancs */
nReteszOffset[104] = 26;             /* DC parancs és DP állapot offsete, ha tartozik hozzá DC parancs*/

/* 105. TMOK: 60-20 RTU: Pereszteg 0160/2 PV erõmû - Tesztelve            -----------------------*/								/**/
TMOKAllasjelzesOffsetek[105] = 1336; 	 /* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[105][0] = 1336;						         /*  TMOK azonosítója a kmenõ táviratban = DP offset */								/**/															
ReteszesRTUIndex[105][0] = 187;			   /* Pereszteg 0160/2 PV erõmû */															/**/
ReteszesTMOK_RTUNum[105] = 1;				   /* Az adott indexû TMOK ennyi kábelköri állomnással kommunikál */			/**/
nReteszPar[105] = 0;                   /* 1: tartozik hozzá DC parancs, 0: nem tartozik hozzá DC parancs */

/* 106. TMOK: 52-48 RTU: TMOK 59297 (Nemesgulács 06/1) - Tesztelve            -----------------------*/								/**/
TMOKAllasjelzesOffsetek[106] = 1337; 	 /* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[106][0] = 1337;						         /*  TMOK azonosítója a kmenõ táviratban = DP offset */								/**/															
ReteszesRTUIndex[106][0] = 180;			   /* 59297 TMOK (Nemesgulács 06/1 PV erõmû) */															/**/
ReteszesTMOK_RTUNum[106] = 1;				   /* Az adott indexû TMOK ennyi kábelköri állomnással kommunikál */			/**/
nReteszPar[106] = 0;                   /* 1: tartozik hozzá DC parancs, 0: nem tartozik hozzá DC parancs */

/* 107. TMOK: Zalaszentgrót DAC RTU: Türje 0135/15, Óhíd 213/8 - Tesztelve            -----------------------*/								/**/
TMOKAllasjelzesOffsetek[107] = 1528; 	 /* Az állásjelzés offsete a DP adatbázisban */								/**/

TMOK_ID[107][0] = 1528;						         /*  TMOK azonosítója a kmenõ táviratban = DP offset */								/**/															
ReteszesRTUIndex[107][0] = 175;			   /* Türje 0135/15 PV erõmû */															/**/

TMOK_ID[107][1] = 1528;						         /*  TMOK azonosítója a kmenõ táviratban = DP offset */								/**/															
ReteszesRTUIndex[107][1] = 184;			   /* Óhíd 213/8 PV erõmû */															/**/
...
/* 110. TMOK: 50-97 RTU: TMKK: Tapolca 0258, 0259 PV erõmû - Tesztelve            -----------------------*/								/**/
TMOKAllasjelzesOffsetek[110] = 1340; 	 /* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[110][0] = 1340;						         /*  TMOK azonosítója a kmenõ táviratban = DP offset */								/**/															
ReteszesRTUIndex[110][0] = 202;			   /* Tapolca 0258, 0259 PV erõmû */															/**/
ReteszesTMOK_RTUNum[110] = 1;				   /* Az adott indexû TMOK ennyi kábelköri állomnással kommunikál */			/**/
nReteszPar[110] = 0;                   /* 1: tartozik hozzá DC parancs, 0: nem tartozik hozzá DC parancs */

/* 111. TMOK: 41-26 RTU: TMKK: Nemesrempehollós 259/7 07/4 PV erõmû - Tesztelve            -----------------------*/								/**/
TMOKAllasjelzesOffsetek[111] = 1341; 	 /* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[111][0] = 1341;						         /*  TMOK azonosítója a kmenõ táviratban = DP offset */								/**/															
ReteszesRTUIndex[111][0] = 203;			   /* Nemesrempehollós 259/7 07/4 PV erõmû */															/**/
ReteszesTMOK_RTUNum[111] = 1;				   /* Az adott indexû TMOK ennyi kábelköri állomnással kommunikál */			/**/
nReteszPar[111] = 0;                   /* 1: tartozik hozzá DC parancs, 0: nem tartozik hozzá DC parancs */
...
{	
	...
	if (TMOKAllasjelzesOffsetek[i]<1750)
	{				
...
/****************************************************************************/
/* MOTOROLA allomas adatfeldolgozas, Tapolca 0258, 0259 PV erõmû miatt											*/
/****************************************************************************/
...
int		nSynchronized;
	...
	p_col_RxBuf = (short *)(rx_buf);	
 		...
 		nMin		 = p_col_RxBuf[25] & 0xff;
		nMs1		 = p_col_RxBuf[26] & 0xff;
		nMs2		 = p_col_RxBuf[26] >>8;
		nSynchronized= p_col_RxBuf[20];
...
if (pMOT->nNMNum > 0)
{
	...
	{
		fnWriteNM( nNMStart+nI,p_col_RxBuf[28+nI]);			
...
if (	nDPStart > 0)
{
	for (nI=0; nI < pMOT->nIEC_DP_NUM && nI < 80; nI++)
	{	
